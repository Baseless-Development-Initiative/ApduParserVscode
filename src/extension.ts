...
import { ApduListUpdate } from './apdu_defs';
...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('Congratulations, your extension "ApduParser" is now active!');

	// The command has been defined in the package.json file
	// Now provide the implementation of the command with registerCommand
	// The commandId parameter must match the command field in package.json
	const disposable = vscode.commands.registerCommand('apduParser.parse', () => {
	...
	const disposable2 = vscode.commands.registerCommand('apduParser.update', () => {
		ApduListUpdate();
	});
